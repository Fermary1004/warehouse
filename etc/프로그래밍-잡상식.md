...
- Application
    - HTTP, FTP, SMTP
    - 사용자에게 보이는 유일한 계층
    - 웹 브라우저, 이메일 클라이언트, 터미널 등
- Presentation
    - JPEG, MPEG, SMP, ASCII, 암호화, 32bit int <-> 64bit int
    - 바이너리 데이터를 의미있는 데이터로 구성하여 표현
- Session
    - TLS(SSH)
    - 포트 연결, 연결 라이프사이클 관리,영상 채팅 시 비디오와 오디오를 한 세션에 묶어준다
- Transport
    - TCP, UDP, ARP, 게이트웨이
    - 메시지가 누락없이 순서에 맞게 전송되는지 검증, 흐름제어, 분할, 재조립, 오류관리
- Network
    - IP, ICMP, IGMP, 라우터
    - 메시지를 패킷으로 분리, 올바른 종착지로 패킷이 전송되도록 물리적 네트워크 사이의 라우팅 책임(PC, 라우터, 스위치 등등)
- Data Link
    - MAC, PPP, FDDI, 브릿지, 스위치
    - 패킷을 프레임으로 분리, raw bytes, 데이터 프레이밍, 에러 디텍션/커렉션,
- Physical
    - Ethernet, 허브, 리피터
    - raw bytes로 된 프레임을 raw bit로 분리, 실제로 물리적 네트워크와 데이터 송수신, 하드웨어 담당
...
- 서버는 서버 자신의 개인키로 session key를 복호화 하고, 복호화 한 session key로 클라이언트가 보낸 실제 데이터를 복호화해서 실제 데이터를 알아낸다.
