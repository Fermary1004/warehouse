...
스코프 체인 자체는 함수의 호출이 아니라 함수의 정의 시에 이미 생성되어, 
함수의 정의 당시의 실행 컨텍스트의 스코프 체인을 가리키고 있다.

즉, 어떤 함수 A가 정의된다는 것은 A를 감싸고 있는 다른 함수 B가 실행 중이라는 것을 의미하며, A가 정의될 때 A의 스코프체인 객체는 B의 실행 컨텍스트 내에 있는 스코프 체인 객체를 가리킨다.

스코프 체인은 변수 객체를 원소로 가지고 있는 링크드 리스트다.

함수 A가 호출되어 실행 컨텍스트가 생기면, 함수 A의 정의 시에 생성된 스코프 체인이 가리키고 있던 모든 변수 객체의 참조를 새로운 링크드 리스트 B에 복사하고, A의 호출에 의해 생성된 실행 컨텍스트내의 변수 객체를 B에 추가하고, A의 실행 컨텍스트에 있는 스코프 체인은 B를 가리키게 된다.

A에서는 링크드 리스트 B에 담긴 모든 변수 객체에 담겨있는 모든 속성(변수와 함수), 즉, A를 포함하여 A를 감싸고 있는 모든 변수와 함수에 접근할 수 있다.  
